  ...
  return (
    <Pressable onPress={() => router.push(`/movies/${movie.id}`)} className="flex-1">
      ...
      <View className="flex-row items-center justify-between mt-2">
        {!!showDate && (
          <Text className="font-body text-sm text-neutral-400">{showDate}</Text>
        )}
        <View className="flex-row items-center">
          <AppIcon icon={StarFillIcon} size={18} color={COLORS.brand.blue100} />
          <Text className="font-body text-sm text-neutral-400">{movie.rating}</Text>
        </View>
      </View>

      <Text className="mt-1 mb-2 text-base font-semibold text-white">{movie.title}</Text>

      <View className="flex-row items-center gap-2">
        {!!movie.genre && <AppTag label={movie.genre} />}
        {!!movie.duration && <AppTag label={movie.duration} />}
      </View>
    </Pressable>
  );
}